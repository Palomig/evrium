<?php
/**
 * OGE Geometry Prototypes - Admin Panel (Edit Mode)
 */
...
<head>
    ...
    <title>Админка — Геометрические прототипы ОГЭ</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        <div class="header">
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                <span class="admin-badge">Режим редактирования</span>
        ...
        <div class="stats">
            ...
            <div>Источник: <span>SQLite</span></div>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadTable() {
            ...
            try {
                ...
                tbody.innerHTML = '<tr><td colspan="5" style="color: #f45c43;">Ошибка: ' + error.message + '</td></tr>';
        ...
        function addRow() {
            const newPrototype = { id: null, num: '', type: '', method: '', image: '' };
        ...
        async function deleteRow(index) {
            ...
            if (prototype.id) {
                try {
                    ...
                    showNotification('Строка удалена', 'info');
                } catch (error) {
        ...
        function removeImage(btn, index) {
            ...
            modifiedRows.add(index);
            btn.closest('tr').classList.add('modified');
        ...
        async function saveTable() {
            ...
            try {
                ...
                showNotification(`Сохранено ${result.saved} записей`, 'success');
                await loadTable();
            } catch (error) {
                showNotification('Ошибка: ' + error.message, 'error');
            }
        ...
        async function resetToDefault() {
            if (!confirm('Сбросить к исходным данным? Все изменения будут потеряны.')) return;

            try {
                prototypes = await apiCall('reset');
                renderTable();
                modifiedRows.clear();
                showNotification('Таблица сброшена', 'info');
            } catch (error) {
                showNotification('Ошибка: ' + error.message, 'error');
            }
        }
