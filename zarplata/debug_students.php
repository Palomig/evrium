        ...
        foreach ($schedule as $key => $entry) {
            if (is_array($entry)) {
                ...
                if ($entryDayNum == $dayOfWeek && substr($entryTime, 0, 5) == $timeStart) {
                    $hasThisLesson = true;
                    echo "<div>✅ Студент {$student['name']} (класс {$student['class']}): Найдено совпадение! День={$entryDay} ({$entryDayNum}), Время={$entryTime}</div>";
                    break;
                }
            } else {
                ...
                if ((int)$key == $dayOfWeek && substr($entry, 0, 5) == $timeStart) {
                    $hasThisLesson = true;
                    echo "<div>✅ Студент {$student['name']} (класс {$student['class']}): Найдено совпадение! День={$key}, Время={$entry}</div>";
                    break;
                }
            }
        }
